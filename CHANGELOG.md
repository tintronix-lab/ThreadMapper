...
- Drive the room-by-room guided survey from the watch: **Start / Done / Skip** the current room, with live progress ("2/5 rooms"), a recording timer, and the current room name — kept in sync with the phone in real time over WatchConnectivity
- Lives on a second watch page (swipe over from the dashboard); a haptic click confirms each command
- Remote-control model: the phone's Guided Survey screen must stay open (it owns the sampling loop) — the watch is a remote, not a background recorder
- `GuidedSurveyBridge` coordinates the phone flow with the watch; `WatchConnectivityManager` gains two-way `sendMessage` (state out, commands in); new `GuidedSurveyControlView` on the watch

**Apple Watch app icon**
- The watch app now ships its own icon — a bolder hub-and-spokes mesh tuned to read at ~30 mm (the 12-node iOS icon is too busy on the wrist). Generated by `gen_watch_icon.py`

### Changed

**Apple Watch UI redesign**
- Rebuilt the watch dashboard: a custom grade ring (grade + score), a colour-coded status line, and full-width Devices / Offline stat tiles
- Everything now fits on one screen — replaced the scrolling layout with a fixed one, so the digital crown no longer hides content below the fold
